    ...
    for i in range(len(tracks)):
        ...
        for t in dirTitles:
            ...
            if tracks[i][0].lower() == fileTitle.lower() or fileTitle.lower() == escapedTrack.lower():
                ...
                if args.overwrite:
                    id3.delete(currentFileName)
                    id3.save(currentFileName)
        ...
        if tries >= len(dirTitles) or tries == len(dirTitles):
            for j in range(len(dirTitles)):
            ...
            while(True):
                ...
                if int(trackSel) >= 0 and int(trackSel) <= len(tracks):
                    ...
                    trackSel = int(trackSel)
                    tries = 0

                    currentFileName = dirTitles[trackSel]
                    replacedFileName = ""
                    numberedFileName = ""
                    ...
                    id3.save(dirTitles[trackSel],v2_version=4)

                    if args.replace:
                        replacedFileName = tracks[int(trackSel)][0]+".mp3"
                        os.replace(currentFileName,replacedFileName)
                        currentFileName = tracks[int(trackSel)][0]+".mp3"
                        print("Renamed: " + currentFileName + " to: " + replacedFileName)

                    if args.numbered:
                        numberedFileName = str(int(trackSel) + 1) + "." + currentFileName
                        os.replace(currentFileName,numberedFileName)
                        currentFileName = numberedFileName + currentFileName
                        print("Numbered: " + currentFileName + " to: " + numberedFileName)
                    break
